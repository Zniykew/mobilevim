...
class LayerNorm2d(nn.Module):
    """
    2D 层归一化模块。
    将输入从 (B, C, H, W) 重排为 (B, H, W, C) 以应用 LayerNorm，
    然后再重排回 (B, C, H, W)。
    """
...
class LayerNorm3d(nn.Module):
    """
    3D 层归一化模块。
    将输入从 (B, C, T, H, W) 重排为 (B, T, H, W, C) 以应用 LayerNorm，
    然后再重排回 (B, C, T, H, W)。
    """
...
def get_norm(norm_layer='in_1d'):
    """
    根据输入字符串返回相应的归一化层。
    支持的归一化层包括 InstanceNorm、BatchNorm、GroupNorm 和 LayerNorm。
    """
    eps = 1e-6
    norm_dict = {
        'none': nn.Identity,
...
def get_act(act_layer='relu'):
    """
    根据输入字符串返回相应的激活函数。
    支持的激活函数包括 ReLU、Sigmoid、Swish、Mish 等。
    """
    act_dict = {
        'none': nn.Identity,
        'sigmoid': Sigmoid,
...
class MSPatchEmb(nn.Module):
    """
    多尺度 Patch Embedding 模块。
    使用不同膨胀率的卷积进行多尺度特征提取，并平均结果。
    """

    def __init__(self, dim_in, emb_dim, kernel_size=2, c_group=-1, stride=1, dilations=[1, 2, 3], norm_layer='bn_2d',
                 act_layer='silu'):
        super().__init__()
